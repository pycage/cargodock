...
quint32 toUint32(const QByteArray& a)
{
    quint32 result = 0;
    for (int i = 0; i < 4 && i < a.size(); ++i)
    {
        result = result << 8;
        result += (unsigned char) a.at(i);
    }
    return result;
}

QByteArray fromUint32(quint32 a)
{
    QByteArray out;
    for (int i = 3; i >= 0; --i)
    {
...
{
    qCopy(sbox0, sbox0 + 256, mySBox0.begin());
    qCopy(sbox1, sbox1 + 256, mySBox1.begin());
    qCopy(sbox2, sbox2 + 256, mySBox2.begin());
    qCopy(sbox3, sbox3 + 256, mySBox3.begin());
    qCopy(parray, parray + 18, myPArray.begin());
}

quint32 Blowfish::f(quint32 x) const
{
    quint32 h = mySBox0[x >> 24] + mySBox1[x >> 16 & 0xff];
    return (h ^ mySBox2[x >> 8 & 0xff]) + mySBox3[x & 0xff];
}

void Blowfish::encryptLR(quint32& l, quint32& r) const
{
    for (int i = 0; i < myPArray.size() - 2; i += 2)
    {
        l ^= myPArray[i];
        r ^= f(l);
        r ^= myPArray[i + 1];
    ...
    l ^= myPArray[myPArray.size() - 2];
    r ^= myPArray[myPArray.size() - 1];
...
void Blowfish::decryptLR(quint32& l, quint32& r) const
{
    for (int i = myPArray.size() - 2; i > 0; i -= 2)
    {
        l ^= myPArray[i + 1];
        r ^= f(l);
        r ^= myPArray[i];
...
void Blowfish::keySchedule(const QByteArray& passphrase)
{
    int keyLength = passphrase.size();
    for (int i = 0; i < myPArray.size(); ++i)
    {
        myPArray[i] ^= toUint32(passphrase.mid((i % keyLength) * 4, 4));
    }

    quint32 l = 0;
    quint32 r = 0;

    for (int i = 0; i < myPArray.size(); i += 2)
    {
        encryptLR(l, r);
        myPArray[i] = l;
        myPArray[i + 1] = r;
    ...
    for (int i = 0; i < mySBox0.size(); i += 2)
    {
        encryptLR(l, r);
        mySBox0[i] = l;
        mySBox0[i + 1] = r;
    ...
    for (int i = 0; i < mySBox1.size(); i += 2)
    {
        encryptLR(l, r);
        mySBox1[i] = l;
        mySBox1[i + 1] = r;
    ...
    for (int i = 0; i < mySBox2.size(); i += 2)
    {
        encryptLR(l, r);
        mySBox2[i] = l;
        mySBox2[i + 1] = r;
    ...
    for (int i = 0; i < mySBox3.size(); i += 2)
    {
        encryptLR(l, r);
        mySBox3[i] = l;
        mySBox3[i + 1] = r;
...
QByteArray Blowfish::encrypt(const QByteArray& data) const
{
    QByteArray output;
    // input is chunks of 64 bit, so iterate over the data
    ...
    for (int i = 0; i < data.size(); i += 8)
    {
        ...
        quint32 l = toUint32(chunk.left(4));
        quint32 r = toUint32(chunk.mid(4, 4));
        encryptLR(l, r);
        output += fromUint32(l) + fromUint32(r);
    }
    return output.toHex();
}

QByteArray Blowfish::decrypt(const QByteArray& hexData) const
{
    QByteArray output;
    ...
    while (output.size() && output.at(output.size() - 1) == null)
    {
        output.chop(1);
    }
    return output;
